...
function doPost(e) {
  logi("I was called appendQuotePart")
  if (typeof e == 'undefined') return 'err;no pars'

  var jsonString = e.postData.getDataAsString();
  var jsonData = JSON.parse(jsonString);

  try {
    var originalPart = jsonData.originalPart
    logi('---appendQuotePart POST\n' + originalPart);
    var transl = LanguageApp.translate(originalPart, '', 'cs');
    logi(transl)
    var sheet = SpreadsheetApp.openById('1ty2xYUsBC_J5rXMay488NNalTQ3UZXtszGTuKIFevOU').getSheetByName('appendQuote');
    sheet.appendRow([originalPart, transl]);
    return buildResponse('', '');
  } catch (e) {
    logi('appendQuotePart POST err:\n' + e)
...
function doGet(e) {
  ...
  try {
    ...
    if (action == 'appendQuoteClear') {
      return appendQuoteClear();
    }

    // ?action=appendQuotePart&originalPart=hello-1
    // ?action=appendQuotePart&originalPart=hello-2
    // ?action=appendQuotePart&originalPart=hello-33
    logi(e.parameter.originalPart);
    logi('--------------originalPart/')

    if (action == 'appendQuotePart') {
      return appendQuotePart(e.parameter.originalPart);
    }

    // ?action=appendQuoteFinal&sheetName=EduardT&sheetId=12ccbLbRfZ94gk-ZTxwj-NXJVNt4oSDjd9mHqxCh-4Uc&parPage=321&author=Tomáš Eduard
    if (action == 'appendQuoteFinal') {
      return appendQuoteFinal(e.parameter.sheetName, e.parameter.sheetId, e.parameter.parPage, e.parameter.author);
    }
