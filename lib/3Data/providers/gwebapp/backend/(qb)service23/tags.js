...
function getrowsByTagPrefix(tagPrefix) {
  try {
    getrowsByTagPrefix_(tagPrefix)
...
function getrowsByTagPrefix_(tagPrefix) {
  ...
  var sheetIndex = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('__tagSheets__')
  var dataf = columnFilter2csv(sheetIndex, 'A1:C', 1, tagPrefix)
  for (var filtIx = 1; filtIx < dataf.length; filtIx++) {
    if (dataf[filtIx][2] == '') continue;
    var sheetName = dataf[filtIx][1];
    var sheetUrl = sheetnamesUrlsMap.get(sheetName);
    var rownos = dataf[filtIx][2].split('_')
    rownos2list(sheetName, sheetUrl, rownos)
  }
}

function getrowsByTagPrefix___test() {
  getrowsByTagPrefix('laska')//'láska'
  log(getrowsByTagPrefixList)
}

function rownos2list(sheetName, sheetUrl, rownos) {
  logi(sheetName + ' ' + rownos)
  try {
    ...
    for (var rowIx = 0; rowIx < rownos.length; rowIx++) {
      var rowNo = rownos[rowIx]
      data[rowNo - 1].unshift(sheetName + '__|__' + rowNo)
      getrowsByTagPrefixList.push(data[rowNo - 1]);
    }
