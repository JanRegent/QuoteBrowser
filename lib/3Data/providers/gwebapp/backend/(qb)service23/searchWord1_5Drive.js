...
function searchWord1_5__(sheetNamesInScope, word1, word2, word3, word4, word5) {
    ...
    var rows1 = fullTextContains_word1(sheetNamesInScope, word1);
    var rows25 = [];
...
function fullTextContains_word1(sheetNamesInScopeStr, word1, sheetNamesInScope) {
    var sheetNamesInScope = getScope(sheetNamesInScopeStr);
    ...
    for (var sheetIx = 0; sheetIx < items.length; sheetIx++) {
        try {
            var rows = searchSS_(word1, items[sheetIx].id);
