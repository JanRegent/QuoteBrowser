...
function tagSheetsIndex() {
  sheetsExists()
  
  var tagsMap  = new Map();
  //-----------------------------------------------------------------get all tags 2 tagMap
  ...
  for (var six =0; six <= sheetNamesSorted.length; six++) {
    ...
    var sheet = SpreadsheetApp.openByUrl(tagsIndexSSurl).getSheetByName(sheetName);
    try {
      var sheetTags = getColumnValuesColIx(sheet,0);
      for (var tix =0; tix <= sheetTags.length; tix++) {
        try {
          var tag = sheetTags[tix];
          var sheetNames = tagsMap.get(tag)
          if (sheetNames == null ) 
            tagsMap.set (tag, sheetName)
          else 
            tagsMap.set(tag, sheetNames + ',' + sheetName)
        }catch{
          continue;
        }
  ...
  for (let key of tagsMap.keys()) {
    try {
      if (key.toString().trim() == '') continue;
    }catch{
      continue;
    }
    try {
    var tag = key.replaceAll(',').trim()
      tagsset.add(tag)
    }catch{}
  }
  var sheet = SpreadsheetApp.openByUrl(tagsIndexSSurl).getSheetByName('__tagSheets__');
  sheet.clear()
  var sortedTags = Array.from(tagsset).sort()
  for (var tix =0; tix < sortedTags.length; tix++) {
    var tag = sortedTags[tix]
    try {
      var sheetNamesArr = tagsMap.get(tag).split(',');
      // var sheetNamesSet  = new Set()
      // for (var six =0; six < sheetNamesArr.length; six++) {
      //   sheetNamesSet.add(sheetNamesArr[six])
      // }
      // sheetNamesArr = Array.from(sheetNamesSet)
      sheet.appendRow([tag,sheetNamesArr.join(',')])
    }catch(e){log(e)}
  }
...
function sheetsIndexTags34() {sheetsIndexTags(31,34)}
function sheetsIndexTags35() {sheetsIndexTags(35,35)} //robertAdams

function sheetsIndexTags40() {sheetsIndexTags(36,40)}
function sheetsIndexTags45() {sheetsIndexTags(41,45)}
function sheetsIndexTags50() {sheetsIndexTags(46,50)}
...
function sheetsIndexTags(startFrom, endTo) {
  ...
  var sheetNamesSorted = Array.from(sheetnamesUrlsMap.keys()).sort()
  log(startFrom +' <->' + endTo + ' len ' + sheetNamesSorted.length)
  ...
  if (endTo >= sheetNamesSorted.length) endTo = sheetNamesSorted.length;
...
function sheetTagIndexCreate(sheet) {
  ...
  for (var rIx =1; rIx < data.length; rIx++) {
    ...
    for (var tIx =0; tIx < tagsArr.length; tIx++) {
      var tag = tagsArr[tIx].trim().replaceAll(',','')
      if (tag == '') continue;
