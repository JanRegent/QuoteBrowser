...
function sheetsIndexTags__trigger() {
  ...
  indexLog = Sheet = SpreadsheetApp.openByUrl(tagsIndexSSurl).getSheetByName('__indexLog__');
  var tagSheet = SpreadsheetApp.openByUrl(tagsIndexSSurl).getSheetByName('__tagSheets__');
  tagSheet.clear();
  indexLog.clear()
...
function sheetTagIndexSave(rownoMap, tagsSet, sheetName) {
  ...
  ixlog('save tagsorted ' + tagsorted.length)
