...
class PrepareKeys {
  ...
  Future sheetAllKeys() async {
    DailyListRow dailyListRow = bl.currentSS.dailyListRow;
    String sheetName = dailyListRow.sheetName;
    List<String> keys = await dl.gservice23.getAllrows(sheetName);
    if (keys.isEmpty) return;
    bl.currentSS.keys = [];
    bl.currentSS.keys.addAll(keys);
    bl.currentSS.swiperIndex.value = int.tryParse(dailyListRow.swiperIndex)!;
  }

  Future getSheetShow(String sheetName, BuildContext context) async {
    bl.homeTitle.value = 'Get sheet \n$sheetName';

    bl.currentSS.keys = await dl.gservice23.getAllrows(sheetName);
